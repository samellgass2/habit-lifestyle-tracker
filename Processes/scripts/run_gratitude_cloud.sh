...
# Path to the processor (match your filename exactly)
PROCESS_PATH="Processes/Gratitude_cloud.py"

# ---------- env for cron ----------
export PATH="/usr/local/bin:/usr/bin:/bin"
# If your brew-installed poetry needs its env, uncomment:
...
ts="$(date '+%F_%H-%M-%S')"
LOG_FILE="$LOG_DIR/gratitude_cloud_$ts.log"
LATEST="$LOG_DIR/gratitude_cloud_latest.log"

{
  echo "=== gratitude_cloud run @ $(date -u '+%F %T')Z on $(hostname) ==="
  echo "whoami: $(whoami)"
  echo "pwd: $(pwd)"
...
# ---------- run processor ----------
set +e
# Pass through any flags you may add later (e.g. --dry-run)
"$POETRY_BIN" run python "$PROCESS_PATH" -vv "$@" 2>&1 | tee -a "$LOG_FILE"
status=${PIPESTATUS[0]}
set -e
...
SUBJECT="[habits] gratitude_cloud $( [ $status -eq 0 ] && echo OK || echo FAIL:$status ) $(date '+%F %T')"
...
#   terminal-notifier -title "Gratitude cloud failed" -message "See $(basename "$LOG_FILE")" -open "file://$LOG_FILE" || true
# fi
